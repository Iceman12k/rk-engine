...
#define Q2PRO_OPTIMIZE(c) \
    (((c)->protocol == PROTOCOL_VERSION_Q2PRO || (c)->protocol == PROTOCOL_VERSION_RK) && !(c)->settings[CLS_RECORDING])
...
=============
*/
static void SV_SendDeltaEntity(edict_t *old, edict_t *new, msgEsFlags_t flags)
{
	entity_packed_t old_pack, new_pack;

	MSG_PackEntity(&old_pack, &old->s, true);
	MSG_PackEntity(&new_pack, &new->s, true);
